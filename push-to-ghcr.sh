...
# GitHub Container Registry (GHCR) Push Script
# This script builds and pushes all CAMP Docker images to GHCR

set -e

# Configuration - UPDATE THESE VALUES
GITHUB_USERNAME="elishatheodore"  # Replace with your GitHub username
REPO_NAME="kubernetes-microservices"    # Repository name
VERSION="latest"                         # You can change this to a specific version
...
if ! docker info 2>/dev/null | grep -q "ghcr.io" && ! grep -q "ghcr.io" ~/.docker/config.json 2>/dev/null; then
    echo "❌ You need to authenticate with GitHub Container Registry first."
    echo "Please run: echo YOUR_GITHUB_TOKEN | docker login ghcr.io -u ${GITHUB_USERNAME} --password-stdin"
    echo ""
    echo "To get a GitHub token:"
    echo "1. Go to GitHub Settings > Developer settings > Personal access tokens"
    echo "2. Generate a new token with 'write:packages' scope"
    echo "3. Use the token as YOUR_GITHUB_TOKEN"
    exit 1
else
    echo "✅ GitHub Container Registry authentication found"
fi
